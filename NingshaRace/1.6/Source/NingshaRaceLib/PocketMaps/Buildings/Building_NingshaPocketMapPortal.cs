...
{
    //类职责：为凝砂族需要分帧生成的原版口袋地图入口提供统一状态和生命周期控制。
    public abstract class Building_NingshaPocketMapPortal : MapPortal
    {
        //字段职责：记录当前入口是否正在生成口袋地图，避免多个进入者重复启动生成流程。
        private bool generationActive;

        //字段职责：记录最近一次生成是否失败，使等待中的进入作业能够结束并允许玩家重试。
        ...
        //函数职责：在进入者完成前摇后启动当前入口对应的分帧口袋地图生成窗口。
