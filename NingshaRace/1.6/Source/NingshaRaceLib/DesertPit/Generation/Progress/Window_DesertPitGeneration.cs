...
namespace NingshaRaceLib.DesertPit.Generation.Progress
{
    ...
    internal sealed class Window_DesertPitGeneration : Window
    {
        ...
        public Window_DesertPitGeneration(Building_DesertPitGate gate)
        {
            this.gate = gate;
        ...
        private void FailGeneration(Exception exception)
        {
            ...
            Messages.Message("沙漠巨坑生成失败，请查看日志后重试。", gate, MessageTypeDefOf.NegativeEvent);
            Log.Error("沙漠巨坑分帧生成失败：" + exception);
            Close(doCloseSound: false);
        }
