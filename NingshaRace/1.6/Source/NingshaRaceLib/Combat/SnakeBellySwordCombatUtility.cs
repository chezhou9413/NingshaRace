...
namespace NingshaRaceLib.Combat
{
    ...
    public static class SnakeBellySwordCombatUtility
    {
        ...
        public static List<IntVec3> FindConeCells(Pawn attacker, LocalTargetInfo primaryTarget, float coneAngle)
        {
            ...
            foreach (IntVec3 cell in GenRadial.RadialCellsAround(attacker.Position, MaxRange, useCenter: false))
            {
        ...
        public static List<Thing> FindTargets(Pawn attacker, LocalTargetInfo primaryTarget, float coneAngle)
        {
            Vector3 attackDirection = HorizontalDirection(attacker.Position.ToVector3Shifted(), primaryTarget.Cell.ToVector3Shifted());
            return FindTargets(attacker, attacker.Map, attacker.Position, attackDirection, coneAngle);
        }

        //函数职责：按固定的挥击起点和方向筛选五格内的 Pawn 和建筑，并按距离从远到近排序。
        public static List<Thing> FindTargets(Pawn attacker, Map map, IntVec3 origin, Vector3 attackDirection, float coneAngle)
        {
            ...
            foreach (IntVec3 cell in GenRadial.RadialCellsAround(origin, MaxRange, useCenter: true))
            {
        ...
        public static DamageWorker.DamageResult ApplyDamage(Verb_SnakeBellySword verb, Thing target)
        {
            ...
            float damageAmount = Rand.Range(props.meleeDamageBaseAmount * 0.8f, props.meleeDamageBaseAmount * 1.2f);
            ...
            {
                armorPenetration = damageAmount * 0.015f;
            }
            else
            {
